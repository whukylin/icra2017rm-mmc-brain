...
#define FIXED_ULTRASONIC_1_PUTBOX 620
	...
	//cout << "ok1!"<<endl;
        ...
        case 1:
            ...
            case 0:
                ...
                if (addboxNum > 0)
                {
                    txKylinMsg_xyz_Fun(tx - (DIFFCONST + (4 - 1 * addboxNum)), X_SPEED_1 * ramp, tz, Y_SPEED_1 * ramp, ry * 3141.592654f / 180.0, Z_SPEED_1_VISION); //
                }
                else
                {
                    txKylinMsg_xyz_Fun(tx - (DIFFCONST + (4 - 1 * boxNum)), X_SPEED_1 * ramp, tz, Y_SPEED_1 * ramp, ry * 3141.592654f / 180.0, Z_SPEED_1_VISION); //
                }
            ...
            case 8:
                ...
                //TODO: 滑台上升位置宏定义
                if (kylinMsg.cbus.gp.e <= GraspBw - GRASP_BOX_POSITION + 10 || (addboxNum > 0 && kylinMsg.cbus.gp.e <= GraspBw - GRASP_BOX_POSITION_ADDBOX + 10))
                {
                    txKylinMsg.cbus.gv.e = 0;
                    lastWs = workState;
                    rstRmp();
            ...
            case 0:
                ...
                else if (boxNum > 4 && addboxNum > 0)
                {
                    if (addboxNum == 3)
                    {
                        ...
                        //到达目的地(基地区位置)
                        //基地区坐标为(AXISX, AXISY)
                        txKylinMsg_xyz_Fun(ADDAXISX + kylinOdomCalib.cbus.cp.x, X_SPEED_3 * ramp, ADDAXISY + kylinOdomCalib.cbus.cp.y, (Y_SPEED_3_FIRSTBOX + ADDSPEED) * ramp, kylinOdomCalib.cbus.cp.z, 0);//Z_SPEED_3 * ramp);
                        txKylinMsg_ec_Fun(0, 0, 0, 0);
